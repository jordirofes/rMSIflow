...
Now that we've seen the normalization methods we can proceed to the visualization of the data. This chunk below allows users to plot m/z intensities of each pixel onto a heatmap with the for of the tissue. You can also choose the images to be plotted by using vectors c(1:6), the normalization parameter can be changed by giving a vector with the normalization values for each pixel (peakM$normalizations includes the normalizations rMSI proc calculated) and you can choose to save plot by changing the save parameter to T.     
...
img2plot <- c(1)          # Writing a vector c() allows you to plot more images at the same time. Input a 0 to plot all the images 
...
For more information we can visualize the raw data by using rMSI functions to calculate de medium spectra of each image in profile format. The medium spectrum is the spectrum that contains the mean of each m/z. The next first chunk will calculate the medium spectra from all images and the second one allows users to compare them.
```{r}
...
scaling  <- T                 # The scaling equalizes the data variance to decrease the bias given the natural difference of 
...
img2plotPca <- 1:6         # Select which images to plot. Using c() allows you to plot more images at the same time. 
...
First of all we will need to choose the number of clusters we will separate our data. For this task we will use the factoextra function fviz_nbclus which allows to use different methods for choosing the adequate number of cluster the data wants.
 - The elbow method (WSS): this method utilizes the within cluster sum of squares (WSS) which represents the intra-cluster variation. The WSS is then calculated for each number of clusters (1, 2, 3,...10). The user should choose the number of clusters so that adding another cluster doesn't reduce as much the WSS. (The elbow of the plot)
 ...
 - The gap method: this method compares the WSS with their expected values under null reference distribution of the data. The user should choose the number of clusters that maximizes the gap stadistic so that it means that the distribution is far from a ran 
```{r}
...
clusterKmeansComparison(peakM, norm, img2plot, groupsImg, clus2comp, kmeansData, save, fileName)
