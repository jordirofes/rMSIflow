...
source(paste0(dir2, "initialPlotter.R")) # Sources all the functions we will need
...
# Important: you will need to put the .tar of the images on the same directory you are working on
...
library(ggplot2)
library(plotly)
medSpecRawplot(wDir, medSpec , img2compareRaw, img2compareRaw2)

```

### Medium spectrum from peak matrix
```{r}
pixels2compare <- c(1:50)
pixels2compare2 <- c(50:100)
name1 <- "Cortex"
name2 <- "Medula"
normalization <- NA
medSpecComp(peakM, pixels2compare, pixels2compare2 ,normalization, name1,name2)
...
pcaD <-pca(data2use, centering, scaling, normalization)
...
plotly::ggplotly(pca2dimPlot(peakM, pcaD, groupsImg, pc2plot1,pc2plot2, save, fileNpca))
...
pcaPlotImg(peakM, matrixList, pcaD, img2plotPca, groupsImg, pc2plot, save, fileNpca2)
